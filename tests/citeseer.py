...
def read_content_graph(graph_name):
    G = nx.Graph()
    node_class = {}
...
def draw_citeseer_embedding_fr(
        d=8,
        walklen=30,
        return_weight=1,
        neighbor_weight=1,
        ...
        te=0.6, wa=1, we=1,):
    graph_file = "citeseer"
    G, node_class = read_content_graph(graph_file)
    class_map = {}
    count = 0
...
def draw_citeseer_cluster_embedding_fr(
        d=8,
        walklen=30,
        return_weight=1,
        neighbor_weight=1,
    ...
    cluster_color = kmeans(vectors, K=count-2)
    cluster_color_list = []
    color_list = []
    for node in G.nodes:
        c = node_class[node]
