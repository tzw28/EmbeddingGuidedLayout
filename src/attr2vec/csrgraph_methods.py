...
@jit(nopython=True, parallel=True, nogil=True, fastmath=True)
def _random_walk(weights, indptr, dst, sampling_nodes, walklen):
    """
...
@jit(nopython=True, nogil=True, fastmath=True)
def _node2vec_inner(
    res, i, k, state, Tdata, Tindices, Tindptr, return_weight, neighbor_weight
):
    """
...
def _node2vec_walks(
    Tdata, Tindptr, Tindices, sampling_nodes, walklen, return_weight, neighbor_weight
):
    """
...
def _attr2vec_first_step(
    state, Tdata, Tindices, Tindptr, virtual_nodes, attribute_weight
):
    """
    Inner code for node2vec walks
    Normal random walk step
    ...
    p0 = p[0]
    this_edges = Tindices[start:end]
    if not _isin(state, virtual_nodes):
        vn = 0
        for ni in this_edges:
            if _isin(ni, virtual_nodes):
                vn += 1
        vir_nodes = np.zeros(vn)
        tempVN = 0
        for ni in this_edges:
            if _isin(ni, virtual_nodes):
                vir_nodes[tempVN] = ni
                tempVN += 1
        for j in vir_nodes:
            index = np.argwhere(this_edges == j)[0]
            p[index] = np.multiply(p0, attribute_weight)
    cdf = np.cumsum(p)
    draw = np.random.rand()
    next_idx = np.searchsorted(cdf, draw)
    state = Tindices[start + next_idx]
...
def _attr2vec_inner(
    ...
    for pe in prev_edges:
        ...
        n_idx = n_idx_v[0]
        p[n_idx] = np.multiply(p[n_idx], neighbor_weight)
    if _isin(state, virtual_nodes):
        for ite in range(len(this_edges)):
            p[ite] = np.multiply(p0, attribute_weight)
    else:
        vn = 0
        for ni in this_edges:
            if _isin(ni, virtual_nodes):
...
@jit(nopython=True, nogil=True, parallel=True, fastmath=True, cache=True)
def _attr2vec_walks(
    Tdata,
    Tindptr,
    Tindices,
    ...
    for i in numba.prange(n_walks):
        ...
        res[i, 0] = state
        # Do one normal step first
        state = _attr2vec_first_step(
            state, Tdata, Tindices, Tindptr, virtual_nodes, attribute_weight
        )
        for k in range(1, walklen - 1):
            # Write state
            res[i, k] = state
            state = _attr2vec_inner(
                res,
                i,
                k,
                state,
